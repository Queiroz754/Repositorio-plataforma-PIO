...
void setup() {
  ...
  strcpy(palavra, "SENAI"); //*sttrcpy compara os vetores
...
//!     ↓ 
  if(strncmp(palavra2, "OLA", 3) == 0) //*strncmp compara os primeiros bites de um vetor com um valor pre determinado  
  {
    Serial.println("As primeiras letras da palavra é OLA");
  }

  if(strstr(palavra2, "MUNDO"))//*procura uma string expecifica dentro de uma string
  {
